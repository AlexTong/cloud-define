...
(function() {
  ...
  (function() {
    ...
    cola.calendar.DateGrid = (function(superClass) {
      ...
      DateGrid.prototype.doFireRefreshEvent = function(eventArg) {
        ...
        if (this._state && this._year && this._month) {
          cellState = this._state[row * 8 + column];
      ...
      DateGrid.prototype.doRefreshCell = function(cell, row, column) {
        var cellState, state, w, xdate, ym;
        state = this._state;
        if (!state) {
          return;
        }
        if (column % 8 > 0) {
          cellState = state[row * 8 + column];
          ...
          cellState = state[row * 8 + column + 1];
          ym = this.getYMForState(cellState);
          xdate = new XDate(ym.year + "-" + (ym.month + 1) + "-" + cellState.text);
          w = DateHelper.getWeekNumber(xdate.toDate());
          return $fly(cell).addClass("week").find(".label").html(w);
        }
      };
      ...
      SwipePicker.prototype.createDateTable = function(dom) {
        ...
        dateTable = new cola.calendar.DateGrid({
          ...
          cellClick: function(self, arg) {
            ...
            cellState = state[arg.row * 8 + arg.column];
            if (cellState.type === "prev-month") {
              calendar.prevMonth();
            } else if (cellState.type === "next-month") {
              calendar.nextMonth();
    ...
    getDateTableState = function(date) {
      var cell, cells, count, day, firstDayPosition, i, index, j, lastM, lastMonthDay, maxDay, x;
      day = date.getDay();
